...
def data_over_time(df,col):
    ...
    nations_over_time.sort_values('Edition')
...
def yearwise_medal_tally(df, country):
    ...
    # Drop duplicates without 'Temp'
    temp_df.drop_duplicates(subset=['NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)
...
def country_event_heatmap(df,country):
    ...
    # Filter data for the selected country
    new_df = temp_df[temp_df['region'] == country]
    heatmap_data = new_df.pivot_table(index='Sport', columns='Year', values='Medal', aggfunc='count', fill_value=0)
    return heatmap_data
